...
# A function is created to filter the datasets
# Create a function to select relevant columns for analysis.
...
# check data summary for NA's presence
summary(combined_data)
# it showed 5197 NA's

# Remove all rows with NA values
filtered_combined_data <- na.omit(combined_data)

# Verify there are no NA values left
anyNA(filtered_combined_data)
...
ggplot(filtered_combined_data, aes(x = country, y = lat, fill = country)) +
...
bins_by_country <- filtered_combined_data %>%
  group_by(country) %>%
...
# bin_uri was not included in the original subset, leading to a different number of bins compared to the original figure.
# The variable names combined_data and data_combined were used inconsistently, leading to errors. They have been standardized throughout the script.
...
ggplot(bins_by_country, aes(x = country, y = num_bins, fill = country)) +
  ...
  scale_fill_manual(values = c("Canada" = "lightblue", "Norway" = "lightcoral"))
# The figure has been improved by modifying font formats.
...
species_by_lat <- filtered_combined_data %>%
...
# Filtered data were used with minor changes to the code
